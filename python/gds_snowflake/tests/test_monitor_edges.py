...
def test_format_account_info_and_notification_reset(mock_vault):
    ...
    # Format account info
    formatted = mon._format_account_info({"current_user": "me", "region": "us"})
    ...
    with patch.object(mon, "replication") as repl:
        ...
        with patch.object(mon, "_check_failover_group_failures", return_value=rr):
            mon.monitor_replication_failures()
        # Next run: resolved
        rr2 = rr.__class__(**{**rr.__dict__, "has_failure": False, "failure_message": None})
        with patch.object(mon, "_check_failover_group_failures", return_value=rr2):
            mon.monitor_replication_failures()
