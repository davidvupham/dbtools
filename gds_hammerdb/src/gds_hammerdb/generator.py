...
class ScriptGenerator:
    ...
    def generate_postgres_tpcc(self, config: HammerDBConfig, pg_config: PostgresConnectionConfig) -> str:
        lines = []
        ...
        if pg_config.password:
            lines.append(self._format_diset("connection", "pg_pass", pg_config.password))
    ...
    def generate_mssql_tpcc(self, config: HammerDBConfig, mssql_config: MSSQLConnectionConfig) -> str:
        lines = []
        lines.append("#!/bin/tclsh")
        lines.append('puts "SETTING CONFIGURATION"')
        ...
        # Connection
        lines.append(self._format_diset("connection", "mssqls_server", mssql_config.server))
        lines.append(self._format_diset("connection", "mssqls_uid", mssql_config.uid))
        if mssql_config.password:
            lines.append(self._format_diset("connection", "mssqls_pass", mssql_config.password))

        lines.append(self._format_diset("tpcc", "mssqls_rampup", config.rampup_minutes))
        lines.append(self._format_diset("tpcc", "mssqls_duration", config.duration_minutes))
    ...
    def generate_postgres_tpch(self, config: HammerDBConfig, pg_config: PostgresConnectionConfig) -> str:
        ...
        # TPC-H Settings
        lines.append(self._format_diset("tpch", "pg_scale_fact", config.tpch_scale_factor))
    ...
    def generate_mssql_tpch(self, config: HammerDBConfig, mssql_config: MSSQLConnectionConfig) -> str:
        ...
        # TPC-H Settings
        lines.append(self._format_diset("tpch", "mssqls_scale_fact", config.tpch_scale_factor))
    ...
    def generate_build_script(self, config: HammerDBConfig, db_config: Any) -> str:
        ...
        if config.db_type == DB_POSTGRES:
            ...
            lines.append(self._format_diset("connection", "pg_user", db_config.superuser))
            if db_config.password:
                lines.append(self._format_diset("connection", "pg_pass", db_config.password))

            if config.benchmark_type == "TPC-C":
                lines.append("dbset bm TPC-C")
                lines.append(self._format_diset("tpcc", "pg_count_ware", 10))
                lines.append(self._format_diset("tpcc", "pg_num_vu", config.virtual_users))
            else:
                lines.append("dbset bm TPC-H")
                lines.append(self._format_diset("tpch", "pg_scale_fact", config.tpch_scale_factor))
                lines.append(self._format_diset("tpch", "pg_num_vu", config.virtual_users))

        elif config.db_type == DB_MSSQL_LINUX:
            lines.append(f"dbset db {DB_MSSQL_LINUX}")
            lines.append(self._format_diset("connection", "mssqls_server", db_config.server))
            lines.append(self._format_diset("connection", "mssqls_uid", db_config.uid))
            if db_config.password:
                lines.append(self._format_diset("connection", "mssqls_pass", db_config.password))

            if config.benchmark_type == "TPC-C":
                lines.append("dbset bm TPC-C")
                lines.append(self._format_diset("tpcc", "mssqls_count_ware", 10))
                lines.append(self._format_diset("tpcc", "mssqls_num_vu", config.virtual_users))
            else:
                lines.append("dbset bm TPC-H")
                lines.append(self._format_diset("tpch", "mssqls_scale_fact", config.tpch_scale_factor))
                lines.append(self._format_diset("tpch", "mssqls_num_vu", config.virtual_users))
